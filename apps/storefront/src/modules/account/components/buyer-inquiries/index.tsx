...
import { BuyerInquiry, replyToBuyerInquiry } from "@lib/data/listing-inquiries"
            ...
            <table className="w-full table-fixed border-collapse text-left">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr className="text-xsmall-semi font-medium uppercase text-ui-fg-subtle">
                  <th className="w-[32%] px-4 py-3">Listing</th>
                  <th className="w-[18%] px-4 py-3">Seller</th>
...
const BuyerInquiryRow = ({
  ...
  const hasUnreadReply = inquiry.status === "replied"
  ...
  return (
    ...
    <td className="px-4 py-3 align-middle">
      <p
        className={`truncate text-small-regular ${
      ...
      >
        {latestMessage
          ? `${latestMessage.sender_type === "seller" ? "Reply: " : ""}${latestMessage.body}`
          : "No messages yet."}
      </p>
    </td>
    <td className="px-4 py-3 align-middle">
    ...
    <td className="px-4 py-3 align-middle">
      <div className="flex items-center justify-end gap-2">
        <button
          ...
          title="Open chat"
          aria-label={`Open chat about ${inquiry.product?.title ?? "listing"}`}
          onClick={onOpenChat}
        >
          <ReplyIcon />
        </button>
...
const BuyerInquiryChatWindow = ({
  ...
  const sellerName = inquiry.product?.seller?.display_name ?? "Seller"

  const handleReply = async (formData: FormData) => {
    await replyToBuyerInquiry(inquiry.id, formData)
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3 border-b border-gray-200 bg-ui-fg-base px-4 py-3 text-white">
        <div className="min-w-0">
          <p className="truncate text-small-semi">{sellerName}</p>
          ...
          inquiry.messages.map((message) => (
            ...
            >
              ...
              >
                ...
                >
                  {message.sender_type === "buyer" ? "You" : sellerName}
                </div>
...
const getLatestMessage = (inquiry: BuyerInquiry) =>
...
const SendButton = () => {
  ...
  return (
    <button
      ...
      title="Send"
      data-testid="buyer-reply-inquiry-button"
    >
      <SendIcon />
    </button>
  )
